...
import { deleteCategoria, getCategorias } from "../../../services/categoriaProducto.services";
import { successToast, errorToast } from "../../../utils/toast";
import ModalConfirm from "../../../components/utils/modals/ModalConfirm";

const CategoriaProductoList = () => {
  const user = useAuthStore((state) => state.user);
  const [items, setItems] = useState<CategoriaProductoType[]>([]);
  const [filtered, setFiltered] = useState<CategoriaProductoType[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [toDelete, setToDelete] = useState<CategoriaProductoType | null>(null);
  ...
  useEffect(() => {
    ...
    const filteredItems = items.filter((it) => {
      return (
        ...
        (it.categoria ?? "").toLowerCase().includes(lower)
      );
    });
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await deleteCategoria(toDelete.id);
      successToast(`Categoría eliminada: ${response?.categoria || toDelete.categoria}`);
      setToDelete(null);
      getList();
    } catch (err: any) {
    ...
    const all = [
      {
        ...
        onClick: (row: CategoriaProductoType) => goTo(`${row.id}/edit`),
        permiso: "categoriaproducto:edit",
      },
      {
        label: (
          <>
        ...
        onClick: (row: CategoriaProductoType) => goTo(`${row.id}`),
        permiso: "categoriaproducto:detail",
      },
      {
        label: (
          <>
        ...
        onClick: (row: CategoriaProductoType) => setToDelete(row),
        permiso: "categoriaproducto:delete",
      },
    ];
    return all.filter((a) => user?.permisos?.includes(a.permiso));
  };

  const getColumns = (): Column<CategoriaProductoType>[] => {
    const base: Column<CategoriaProductoType>[] = [
      { id: "categoria", label: "Categoría", minWidth: 200 },
  ...
  return (
    <>
      <Grid container spacing={2} flexGrow={1} size={12} width={"100%"}>
        <Grid flexGrow={1} container p={1} gap={2} justifyContent={{ sm: "center", md: "flex-end" }}>
          <Grid size={{ xs: 8, md: 8 }} display={{ xs: "flex" }} flexGrow={1} alignItems={"center"} justifyContent={"end"}>
            <Search onSearch={setSearchTerm} placeholder="Buscar categorías..." />
          </Grid>
        ...
        <Grid size={12}>
          <TableCustom<CategoriaProductoType> columns={columns} rows={filtered} />
        </Grid>
      </Grid>
      ...
      <ModalConfirm
        ...
        text={`¿Seguro que deseas eliminar la categoría "${toDelete?.categoria}"? Esta acción desactivará el registro.`}

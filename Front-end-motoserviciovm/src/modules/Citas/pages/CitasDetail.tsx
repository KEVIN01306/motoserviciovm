                ...
                <Box>
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>
                    {(cita?.cliente ? (
                      <LinkStylesNavigate
                        variant="h5"
                        onClick={() => goTo('/admin/users/' + cita?.cliente?.id)}
                        label={`${cita?.cliente?.primerNombre || ''} ${cita?.cliente?.primerApellido || '-'}`}
                      />
                    ) : (
                      cita?.nombreContacto || '-'
                    ))}
                  </Typography>         <Typography color="text.secondary">{cita?.telefonoContacto ?? '-'}</Typography>

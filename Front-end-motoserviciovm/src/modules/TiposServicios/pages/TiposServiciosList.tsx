...
import { deleteTipo, getTipos } from "../../../services/tipoServicio.services";
import { successToast, errorToast } from "../../../utils/toast";
import ModalConfirm from "../../../components/utils/modals/ModalConfirm";

const TiposServiciosList = () => {
  const user = useAuthStore((state) => state.user);
  const [items, setItems] = useState<TipoServicioGetType[]>([]);
  const [filtered, setFiltered] = useState<TipoServicioGetType[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [toDelete, setToDelete] = useState<TipoServicioGetType | null>(null);
  ...
  useEffect(() => {
    ...
    const filteredItems = items.filter((it) => {
      return (
        ...
        (it.tipo ?? "").toLowerCase().includes(lower) ||
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await deleteTipo(toDelete.id);
      successToast(`Tipo eliminado: ${response?.tipo || toDelete.tipo}`);
      setToDelete(null);
      getList();
    } catch (err: any) {
    ...
    const all = [
      {
        ...
        onClick: (row: TipoServicioGetType) => goTo(`${row.id}/edit`),
        permiso: "tiposervicios:edit",
      },
      {
        label: (
          <>
        ...
        onClick: (row: TipoServicioGetType) => goTo(`${row.id}`),
        permiso: "tiposervicios:detail",
      },
      {
        label: (
          <>
        ...
        onClick: (row: TipoServicioGetType) => setToDelete(row),
        permiso: "tiposervicios:delete",
      },
    ];
    return all.filter((a) => user?.permisos?.includes(a.permiso));
  };

  const getColumns = (): Column<TipoServicioGetType>[] => {
    const base: Column<TipoServicioGetType>[] = [
  ...
  return (
    <>
      <Grid container spacing={2} flexGrow={1} size={12} width={"100%"}>
        <Grid flexGrow={1} container p={1} gap={2} justifyContent={{ sm: "center", md: "flex-end" }}>
          <Grid size={{ xs: 8, md: 8 }} display={{ xs: "flex" }} flexGrow={1} alignItems={"center"} justifyContent={"end"}>
            <Search onSearch={setSearchTerm} placeholder="Buscar tipos de servicio..." />
          </Grid>
          {user?.permisos?.includes("tiposervicios:create") && (
            <Grid size={{ xs: 1, md: 1 }} display={{ xs: "flex" }} flexGrow={1} alignItems={"center"} justifyContent={"end"}>
              <Fab size="small" color="primary" aria-label="add" onClick={() => goTo("/admin/tiposervicio/create")}>
                <AddIcon />
              </Fab>
            </Grid>
          )}
        ...
        <Grid size={12}>
          <TableCustom<TipoServicioGetType> columns={columns} rows={filtered} />
        </Grid>
      </Grid>
      ...
      <ModalConfirm
        ...
        text={`¿Seguro que deseas eliminar el tipo "${toDelete?.tipo}"? Esta acción desactivará el registro.`}
...
export default TiposServiciosList;
